...
const createUserIcon = (heading: number | null | undefined, compassHeading: number | null | undefined) => {
  // Use compassHeading if available, fallback to movement heading
  const rotation = compassHeading ?? heading ?? 0;
  const showBeam = (compassHeading !== undefined && compassHeading !== null) || (heading !== undefined && heading !== null);
...
const UserLocationMarker = ({ location }: { location: MapProps['currentLocation'] }) => {
  ...
  return (
    <Marker 
      ...
      icon={createUserIcon(location.heading, location.compassHeading)}
      zIndexOffset={1000}
    />
  );
};
        ...
        {typeof window !== 'undefined' && 
         ...
         typeof DeviceOrientationEvent.requestPermission === 'function' && (
          <button 
            ...
            className="bg-white p-3 rounded-full shadow-lg border border-slate-200 text-slate-500 active:scale-90 transition-transform"
